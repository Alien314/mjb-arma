...
private _index = ("true" configClasses (configFile >> "tmf_adminmenu_authorized_players")) findIf {getText (_x >> "uid") isEqualTo getPlayerUID _unit};
...
switch true do {
    ...
    case (_index != -1);
	case (tmf_zeusEnableAdmin && {!(isNull (getAssignedCuratorLogic player))});
