...
if (ctab_core_bft_mode > 0) then {
	/*
	cTabBFTmembers --- GROUP MEMBERS
	*/
	{
		if ((_x isNotEqualTo cTab_player || {mjb_ctab_alwaysDrawPlayer}) && {[_x,ctab_core_personnelDevices] call cTab_fnc_checkGear}) then {
...
if (ctab_core_helmetcam_mode == 1) then {
	/*
...
if (cTabUAVlist isNotEqualTo _cTabUAVlist) then {
	cTabUAVlist = _cTabUAVlist;
	_updateInterface pushBack ["uavListUpdate",true];
};
if (cTabHcamlist isNotEqualTo _cTabHcamlist) then {
...
["ctab_listsUpdated"] call CBA_fnc_localEvent;
