...
if (mjb_tacReload) then {
	mjb_tacLoadHash = createHashMap;
	["CAManBase", "MagazineReloading", {
		...
		if ((_unit isNotEqualTo (call CBA_fnc_currentUnit)) || {
			...
			(mjb_tacLoadHash getOrDefaultCall [(_class + '_isBelt'), {
				if (missionNamespace getVariable ['ace_reload',false]) exitWith {getNumber (configFile >> "CfgMagazines" >> _class >> 'ACE_isBelt')};
				parseNumber ('belt' in toLower _class || {
			...
			(mjb_tacLoadHash getOrDefault [(_class + '_count'), getNumber (configFile >> "CfgMagazines" >> _class >> 'count'), true]) isEqualTo 1} } } }
		) exitWith { };
		...
		private _aID = (_unit addAction ['Quick Reload', {
			...
			private _evID = (_unit addEventHandler ["Reloaded", {
				...
				if (!isNil '_oldMagazine') then {
					...
					if (!mjb_tacDropEmptyMagazines && {_ammo isEqualTo 0}) exitWith { _unit removeEventHandler [_thisEvent,_thisEventHandler]; };
					...
					_deathBox setPosASL (_unit modelToWorldVisualWorld (_pos));
			...
			[{
				params ['_unit','_aID'];
				_unit removeEventHandler ['Reloaded', _aID];
			},[_unit,_evID],((_unit getUnitMovesInfo 7) * 0.61)] call CBA_fnc_waitAndExecute;
			_unit removeAction _aID;
		},_muzzle,-1,false,true,'reloadMagazine']);
		[{params ['_unit','_aID']; _unit removeAction _aID;},[_unit,_aID],0.6] call CBA_fnc_waitAndExecute;
	}, true, [], true] call CBA_fnc_addClassEventHandler;
};
