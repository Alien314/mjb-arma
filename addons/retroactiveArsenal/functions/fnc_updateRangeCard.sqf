...
params ["_zeroRange", "_boreHeight", "_ammoClass", "_magazineClass", "_weaponClass", "_muzzleClass"];
...
if (_muzzleVelocity == 0) then {
    ...
    private _muzzleCoef = getNumber (configFile >> "CfgWeapons" >> _muzzleClass >> "ItemInfo" >> "MagazineCoef" >> "initSpeed");
	if (_muzzleCoef isEqualTo 0) then {_muzzleCoef = 1.0;};
    _muzzleVelocity = _initSpeed * _muzzleCoef;
};
...
private _cacheEntry = missionNamespace getVariable format [QGVAR(%1_%2_%3_%4_%5_%6_%7%8), _zeroRange, _boreHeight, _ammoClass, _weaponClass, _isABenabled, _useBarrelLengthInfluence, _useAmmoTemperatureInfluence, _muzzleClass];
if (isNil "_cacheEntry") then {
    ...
    missionNamespace setVariable [format [QGVAR(%1_%2_%3_%4_%5_%6_%7%8), _zeroRange, _boreHeight, _ammoClass, _weaponClass, _isABenabled, _useBarrelLengthInfluence, _useAmmoTemperatureInfluence, _muzzleClass], [GVAR(rangeCardDataElevation), GVAR(rangeCardDataWindage), GVAR(rangeCardDataLead), GVAR(rangeCardDataMVs), GVAR(lastValidRow)]];
} else {
