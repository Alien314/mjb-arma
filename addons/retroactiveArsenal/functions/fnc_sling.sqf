...
if (_sling) then {
	...
	tsp_future pushBack [time + _time + 0.2, [_unit, !_drawPistol && {!_drawLauncher && {!_unsling} }], {
		...
		_unit setVariable ["tsp_holder", [_unit, primaryWeapon _unit, true, true, "tsp_holder", !_unarmed, isNil "tsp_server_animate" || vehicle _unit isNotEqualTo _unit] call tsp_fnc_throw];
		...
		if (_unarmed && {vehicle _unit isEqualTo _unit}) then {[_unit, [animationState _unit regexReplace ["wrfl", "wnon"] regexReplace ["sras", "snon"] regexReplace ["slow", "snon"], 0, 0, false]] remoteExec ["switchMove"]};
	}];
	_time = _time + 0.2;
};
if (_holster) then {
