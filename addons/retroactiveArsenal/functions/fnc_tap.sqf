params ["_unit", ["_target", objNull], ["_side", -1], ["_mode", if ((getCameraViewDirection (_this#0))#2 < -0.35) then {"leg"} else {"shoulder"}]];
	...
	if (_mode isEqualTo "head" || {(getCameraViewDirection (_this#0))#2 > -0.05 && {((positionCameraToWorld [0,0,0]) distance (_target modelToWorld selectionPosition [_target, "head", 12, true, "FirstPoint"])) < 1.5}}) exitWith {[ACE_player, _target] spawn hfgb_fnc_patHead;};
