		...
		[[], {
			...
			0 spawn {
				...
				if !(isNil 'arsenal') then {
					[arsenal, _magnified] call ace_arsenal_fnc_addVirtualItems;
				};
				if !(isNil 'missionArsenal') then {
					[missionArsenal, _magnified] call ace_arsenal_fnc_addVirtualItems;
				};
	...
	[[], {
		...
		0 spawn {
			...
			mjb_disableMagEH = (["ace_arsenal_boxInitialized", {
				params ["_box"];
				[_box, _magnified] call ace_arsenal_fnc_removeVirtualItems;
