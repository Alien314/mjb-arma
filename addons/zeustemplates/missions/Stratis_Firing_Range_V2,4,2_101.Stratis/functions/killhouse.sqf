tsp_fnc_killhouse_menu = {
	...
	_title = _display ctrlCreate ["RscText", 9998]; _w = 0.4; _h = 0.042; _y = 0.36; _title ctrlSetPosition [(1.0-_w)*0.5,((1.0-_h)*0.5)-_y,_w,_h];
...
if (isClass (configFile >> 'CfgPatches' >> 'tsp_breach')) then {
    ...
    while {_display isNotEqualTo displayNull} do {if (_start getVariable ['generating', false]) exitWith {_display closeDisplay 1;}};
};
...
tsp_fnc_killhouse = {
    params [
        ...
        ["_lockChance", 0], ["_openingChance", 0.2], ["_doorChance", 0.95], 
        ["_wallTypes", ["Bro_MSW_2m"]], ["_outerWallType", "Bro_MSW_2m_conc"], ["_wallLength", 2],
        ["_doorTypes", ["Bro_MSW_2m_d", "Bro_MSW_2m_d", "Bro_MSW_2m_de"]], ["_openDoorTypes", ["Bro_MSW_2m_de"]], ["_outerDoorTypes", ["Land_Bro_MSW_2m_d_conc"]], 
    ...
    _enterance = (_start nearObjects ["Land_Bro_MSW_2m_d_conc", 5])#0;
    _blocker = createSimpleObject ["A3\structures_f\data\DoorLocks\planks_1.p3d", getPosASL _start]; _blocker attachTo [_enterance, [0,-0.2,-1.3]]; //!-- _enterance is undefined, somehow
