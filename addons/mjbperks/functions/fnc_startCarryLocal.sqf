...
if (ace_dragging_doPickup) then {
	...
	[QEGVAR(common,setAnimSpeedCoef), [_unit, (10/ace_dragging_pickupTime)]] call CBA_fnc_globalEvent;
	[QEGVAR(common,setAnimSpeedCoef), [_target, (10/ace_dragging_pickupTime)]] call CBA_fnc_globalEvent;
};
    [_target, "AinjPfalMstpSnonWrflDnon_carried_Up", 2] call EFUNC(common,doAnimation);

    _timer = ([1,CBA_missionTime + ace_dragging_pickupTime] select ace_dragging_doPickup);
} else {
