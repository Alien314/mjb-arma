...
_position = _position vectorAdd [0, ([0,1] select (stance _unit isEqualTo "PRONE")), _offset];
...
if (_target isKindOf "CAManBase") then {
	if (stance _unit isEqualTo "PRONE") then {[_target, "unconscious", 2] call EFUNC(common,doAnimation);
    } else { [_target, "AinjPpneMrunSnonWnonDb_still", 0] call EFUNC(common,doAnimation); };
};
...
// Block firing
if (!GVAR(dragAndFire) || {!GVAR(dragAndFireProne) && {stance _unit isEqualTo "PRONE"}}) then {
