...
if (ace_dragging_doCarry) then {
    [_unit, "AcinPercMstpSnonWnonDnon", 2] call EFUNC(common,doAnimation);
	[QEGVAR(common,setAnimSpeedCoef), [_unit, (ace_dragging_carrySpeedCoef)]] call CBA_fnc_globalEvent;
	[QEGVAR(common,setAnimSpeedCoef), [_target, (ace_dragging_carrySpeedCoef)]] call CBA_fnc_globalEvent;
} else {[_unit, "blockSprint", QUOTE(ADDON), true] call EFUNC(common,statusEffect_set);};
