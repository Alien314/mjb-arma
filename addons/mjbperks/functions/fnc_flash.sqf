...
private _statement = {
	...
	if (typeOf _house isEqualTo "Land_Carrier_01_island_01_F") then {
    ...
    [{
		...
		[{params["_reset"];
			if (_reset isEqualTo "wbk_fists_attack_leg") then {_reset = ""}; player switchMove _reset},
		[_curAnim], 1] call CBA_fnc_waitAndExecute;
	}, [], 0.69] call CBA_fnc_waitAndExecute;
    [{params ["_detonate", "_house","_animations"];
		[_detonate] call ace_grenades_fnc_flashbangThrownFuze;
