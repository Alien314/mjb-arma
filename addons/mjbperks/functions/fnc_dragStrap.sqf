params ['_unit',player],'_target',[cursorObject]];
...
mjb_dragStrapPFH = ([{ params ['_args','_handle'];
	_args params ['_unit','_target','_start','_end','_rope'];
	if (isNil "mjb_dragDummyTg") exitWith {
		_unit forceWalk false;
		[_handle] call CBA_fnc_removePerFrameHandler;
	};
	private _aPos = (_unit worldToModelVisual (getPos _end));
    _aPos vectorDiff (_unit selectionPosition "pelvis");
	_aPos set [2,((_aPos # 2) + 0.3)];
	_target attachTo [_unit, _aPos,"Pelvis",false];
	if ((_start distance _end) > ((ropeLength _rope) - 0.5)) then {
		_unit forceWalk true;
		_target setDir ( (getDir _target + (_target getRelDir (getPos _unit) ) ) mod 360);
	} else { _unit forceWalk false; };
}, 0.1, [_unit,_target,mjb_dragDummy,mjb_dragDummyTg,mjb_rope]] call CBA_fnc_addPerFrameHandler);
