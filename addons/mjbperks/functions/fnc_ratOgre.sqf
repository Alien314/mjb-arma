...
if (_perkName in mjb_activePerks) exitWith { 
	...
	EM_default_animspeedcoef = MJB_defaultSpeed;
	player setAnimSpeedCoef EM_default_animspeedcoef;
};
// perk effects
mjb_defaultHP = _target getVariable ["diw_armor_plates_main_maxHp", diw_armor_plates_main_maxPlayerHP];
private _newHP = (ceil (mjb_defaultHP * (mjb_extraHpMult/2)));
_target setVariable ["diw_armor_plates_main_maxHp", _newHP, true];
...
MJB_defaultSpeed = EM_default_animspeedcoef;
EM_default_animspeedcoef = (EM_default_animspeedcoef * mjb_ratOgreMult);
player setAnimSpeedCoef EM_default_animspeedcoef;
